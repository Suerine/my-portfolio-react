...
const Projects = () => {
 ...
 const filteredProjects = activeCategory === 'All'
  ...
  if (scrollContainerRef.current) {
    ...
    if (card) {
      const cardWidth = card.offsetWidth + 24; // include gap (gap-6 = 24px)
      container.scrollTo({
  ...
  return (
    <section id="projects" className='relative py-20 bg-black overflow-hidden'>
     <div className='absolute inset-0 overflow-hidden'>
       <div className='absolute top-1/3 right-0 w-96 h-96 bg-primary/20 opacity-0 rounded-full blur-3xl'/>
      ...
      <div className="relative">
       ...
       {filteredProjects.length > 3 && (
        ...
        <button 
         onClick={nextSlide}
         disabled={
           currentIndex >= filteredProjects.length - getVisibleCards()
         }
